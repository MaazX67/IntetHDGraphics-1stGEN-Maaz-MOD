...
void ShowMenu(HWND window) noexcept {
    ...
    const UINT fpsFlags = MF_STRING | (MaazXGPU::Perf::IsFpsBoostEnabled() ? MF_CHECKED : MF_UNCHECKED);
    AppendMenuW(menu, fpsFlags, ID_FPS_BOOST, L"FPS Boost");
    AppendMenuW(menu, MF_STRING, ID_MINECRAFT, L"Boost Minecraft javaw.exe");
    ...
    if (message == WM_COMMAND) {
        ...
        case ID_MINECRAFT:
            ShowStatus(window, L"MaazXGPU", MaazXGPU::Perf::ApplyHighPerformanceToMinecraft()
                ? L"HIGH_PERFORMANCE priority applied to Minecraft javaw.exe."
        ...
        case ID_BALANCE:
            ...
            return 0;
        case ID_ABOUT:
            ShowStatus(window, L"MaazXGPU", L"User-mode compatibility layer; the Intel kernel display driver remains installed.");
            return 0;
        case ID_EXIT:
            PostQuitMessage(0);
            return 0;
